...
function Header({history}) {
    const {isAuthenticated, setIsAuthenticated, cartst, setCartst, setCartView, cart} = useAuthentication()
    ...
    return (<>
            {!burger && <div className={burger ? "" : 'burger__menu'}>
                {isAuthenticated && <Link to={"/userdata"} className={"header__text"} onClick={changeBurgerToggle}>Ваши
                    контакты</Link>}
                <Link to={'/cart'}>
                    <div onClick={changeBurgerToggle} className={"header__text"}>Оформить
                        заказ
                    </div>
                </Link>
                <Link onClick={changeBurgerToggle} to={"/products"} className={"header__text"}>
                    <div>Продукты</div>
                </Link>
                <Link onClick={changeBurgerToggle} className={"header__text"} to={"/aboutus"}>
                    <div>О нас</div>
                </Link>
                {!isAuthenticated &&
                <Link to={"/loginpage"}>
                    <div onClick={changeBurgerToggle} className={"header__text"}>Log in</div>
                </Link>}
                {isAuthenticated &&
                <Link to={"/loginpage"}>
                    <div onClick={logOut} className={"header__text"}>Log
                        out
                    </div>
                </Link>}
                {!isAuthenticated &&
                <Link to={"/users"}>
                    <div onClick={changeBurgerToggle} className={"header__text"}>Регистрация</div>
                </Link>}
                {isAuthenticated &&
            ...
            <div className={"header"}>
                <Link to={"/main"}><img src={logo} className={"header__logo"}/></Link>
                {isAuthenticated && <NavLink to={"/userdata"} className={"header__text"}>Ваши
                    контакты</NavLink>}
                <NavLink className={"header__text"} to={'/cart'}>Оформить
                    заказ</NavLink>
                ...
                {isAuthenticated &&
                <Badge className={"badge"} count={cart.length}> <img onClick={toggleCart} className={"header__logo"}
                                                                     src={cartimg}/></Badge>}
            </div>
        </>
    )
